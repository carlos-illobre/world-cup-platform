interface BarGroupProps {
  title: string;
  series: { label: string; values: number[]; tint: "blue" | "green" | "mixed" }[];
}

function bars(values: number[], tint: "blue" | "green" | "mixed") {
      ...
      {values.map((v, i) => {
        ...
        return (
          <div
            ...
            style={{ height: `${Math.max(8, h)}%`, background: bg }}
          />
        );
      })}
    </div>
...
export function BarChart({ title, series }: BarGroupProps) {
  return (
    <div>
