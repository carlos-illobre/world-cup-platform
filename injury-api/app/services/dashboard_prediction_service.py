...
# <-- AGREGAR ESTA LÍNEA PARA CREAR EL LOGGER
logger = logging.getLogger(__name__)


class CachedPrediction:
    """Estructura para almacenar predicción con timestamp."""
    def __init__(self, prediction: DashboardPredictionResponseSchema):
...
class DashboardPredictionService:
    ...
    PREDICTION_TTL_SECONDS: int = 300  # 5 minutos
    
    # Caché con TTL
    _prediction_cache: Dict[Tuple[str, int], CachedPrediction] = field(default_factory=dict, init=False, repr=False)
    ...
    def build_dashboard_prediction(
        ...
        """Ejecuta inferencia y mapea el resultado al payload del dashboard, con caché y TTL."""
        
        cache_key = (player_name, match_number)
        current_time = time()
        
        # Verificar si tenemos predicción cacheada y si aún es válida
        if cache_key in self._prediction_cache:
            cached = self._prediction_cache[cache_key]
            age = current_time - cached.timestamp
            
            if age < self.PREDICTION_TTL_SECONDS:
                logger.debug(
                    f"📦 Predicción cacheada para {player_name} - partido {match_number} "
                    f"(edad: {age:.0f} segundos, TTL: {self.PREDICTION_TTL_SECONDS}s)"
                )
                return cached.prediction
            else:
                logger.info(
                    f"⏰ Predicción expirada para {player_name} - partido {match_number} "
                    f"(edad: {age:.0f}s > TTL: {self.PREDICTION_TTL_SECONDS}s) - recalculando..."
                )
                del self._prediction_cache[cache_key]
        
        # Si no está en caché o expiró, calcular
        logger.info(f"🔮 Calculando predicción para {player_name} - partido {match_number}")
        ...
        result = DashboardPredictionResponseSchema(
        ...
        self._prediction_cache[cache_key] = CachedPrediction(result)
    ...
    def clear_prediction_cache(self) -> None:
        """Limpia el caché de predicciones (útil para testing o recarga forzada)."""
    ...
    def build_batch_dashboard_predictions(
        ...
        """
        Construye predicciones para múltiples jugadores en el mismo partido.
        Usa procesamiento batch para máxima eficiencia.
        """
        ...
        for player_name in player_names:
            ...
            if cache_key in self._prediction_cache:
                ...
                age = current_time - cached.timestamp
                if age < self.PREDICTION_TTL_SECONDS:
                    results[player_name] = cached.prediction
                    continue
                else:
                    del self._prediction_cache[cache_key]
        ...
        # Procesar batch de jugadores restantes
        logger.info(f"🚀 Procesando batch de {len(players_to_process)} jugadores para partido {match_number}")
        
        # Obtener predicciones core en batch
        core_predictions = self.injury_prediction_service.predict_batch_injury_risk(
            player_names=players_to_process,
            match_number=match_number,
        )
        
        # Construir respuestas del dashboard para cada uno
        for player_name, core_prediction in core_predictions.items():
            # Obtener datos del jugador y sensores (estos no se pueden batch fácilmente por ahora)
            player_row = self._find_player_row(player_name)
            sensor_row = self._find_sensor_row(player_name)
            risk_level = InjuryRiskLevel(core_prediction.injury_risk.risk_level)
